...
public class RoomTypeController {
    ...
    public ResponseEntity<?> listAllBedTypes(@PageableDefault(page = 0, size = 5) Pageable pageable)
    {
        ...
        List<RoomType> allRoomTypes = roomTypeService.findAll(pageable);
        return new ResponseEntity<>(allRoomTypes, HttpStatus.OK);
    }

    @GetMapping(value = "room/{room_id}", produces = {"application/json"})
    public ResponseEntity<?> listGroupById(@PathVariable Long room_id)
    {
        RoomType roomType = roomTypeService.findById(room_id);
