...
import com.lambda.bw_airbnb_price_opt.service.PropertyTypeService;
...
public class PropertyTypeController {
    ...
    public ResponseEntity<?> listAllBedTypes(@PageableDefault(page = 0, size = 5) Pageable pageable)
    {
        ...
        List<PropertyType> allPropertyTypes = propertyTypeService.findAll(pageable);
        return new ResponseEntity<>(allPropertyTypes, HttpStatus.OK);
    }

    @GetMapping(value = "property/{property_id}", produces = {"application/json"})
    public ResponseEntity<?> listGroupById(@PathVariable Long property_id)
    {
        PropertyType propertyType = propertyTypeService.findById(property_id);
