...
public class NeighbourHoodGroupController {
    @Autowired
    NeighbourHoodGroupService neighbourHoodGroupService;

    @ApiOperation(value = "Returns all Neighbourhood Groups with pagin ability", responseContainer = "List")
...
//            @ApiResponse(code = 201, message = "List of all rr", response = NeighbourHoodGroup.class),
    ...
    public ResponseEntity<?> listGroupById(@PathVariable Long group_id)
    {
        NeighbourHoodGroup group = neighbourHoodGroupService.findGroupById(group_id);
    ...
    public ResponseEntity<?> addNewGroup(@Valid @RequestBody NeighbourHoodGroup newGroup) throws URISyntaxException
    {
        ...
        URI newGroupURI = ServletUriComponentsBuilder
                .fromCurrentRequest().path("/{group_id}").buildAndExpand(newGroup.getNeighbourhood_group_id()).toUri();
    ...
    @PutMapping(value = "group/{group_id}")
    public ResponseEntity<?> editNewGroup(@Valid @RequestBody NeighbourHoodGroup newGroup, @PathVariable long group_id) throws URISyntaxException
    {
        neighbourHoodGroupService.update(newGroup, group_id);
    ...
    public ResponseEntity<?> deleteNewGroup(@PathVariable long group_id) throws URISyntaxException
    {
        neighbourHoodGroupService.delete(group_id);
        return new ResponseEntity<>(HttpStatus.ACCEPTED);
    }
